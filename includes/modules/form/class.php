...
if(!class_exists('Class_My_Module_Form_Static')){
	class Class_My_Module_Form_Static{
		...
		static function password($name,$element){
			$type=$element['type'];
			$dir=plugin_dir_path(__FILE__).'/views/';
			$file=$dir.$type.'.php';
			extract($element);
		...
		static function font($name,$element){
			...
			$new_element['default']=$default[$k];
				...
				if($k=='font'){
					...
					foreach($wp_my_sogrid_fonts as $k=>$v){
					...
					$start=$wp_my_sogrid_font_sizes_array['start'];
					$step=$wp_my_sogrid_font_sizes_array['step'];
					$end=$wp_my_sogrid_font_sizes_array['end'];
					...
					$values=$wp_my_sogrid_font_styles;
					$new_element['styles']['width']='90px';
					$new_element['values']=$values;
				}else if($k=='font_weight'){
					$values=$wp_my_sogrid_font_weight;
					$new_element['values']=$values;
					$new_element['styles']['width']='90px';
