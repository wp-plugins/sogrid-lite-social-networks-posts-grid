...
if(!class_exists('Class_Wp_My_SoGrid_Table_View')){
	class Class_Wp_My_SoGrid_Table_View{
		var $colums;
		var $data;
		var $search_form;
		var $search_title;
		...
		function Class_Wp_My_SoGrid_Table_View($params=array()){
		...
		function render($echo=false){
			ob_start();
				...
				if(empty($this->data['count'])){
					?>
					<div class="my_error_1"><?php echo __("There are no saved SoGrids !","sogrid_object_meta");?></div>
					<?php
				}else {
		...
		private function render_pagination(){
			...
			<?php echo __("Page","sogrid_object_meta").':'.$this->data['page'];?>&nbsp;
			<?php echo __("Pages","sogrid_object_meta").':'.$this->data['pages'];?>&nbsp;
			<?php echo __("Total Results","sogrid_object_meta").':'.$this->data['count'];?>&nbsp;
				<ul>
					<?php if($this->data['page']>1){?>
						<li><a href="#javascript" class="my_pagination_<?php echo $this->id?>" my_page="1" my_id="<?php echo $this->id;?>"><?php echo __("First Page","sogrid_object_meta");?></a></li>
					<?php }?>
					<?php if($this->data['page']>1){?>
					...
					<?php }?>
					<?php if($this->data['page']<$this->data['pages']){?>
						<li><a href="#javascript" class="my_pagination_<?php echo $this->id?>" my_page="<?php echo $this->data['pages'];?>" my_id="<?php echo $this->id;?>"><?php echo __("Last Page","sogrid_object_meta");?></a></li>
					<?php }?>
			...
			<table class="widefat">
				<thead>
					...
					<?php if(!empty($this->actions)){?>
							<th width="35%"><?php echo __("Actions","sogrid_object_meta");?></th>
						<?php }?>
					</tr>
			...
			<?php 
				...
				if($k%2)$class="alternate";
					...
					<tr class="<?php echo $class;?>" id="tr_id_<?php  echo $v->ID?>">
						...
						<?php if(!empty($this->actions)){?>
							<td>
								<a href="<?php $url=admin_url('admin.php?page=my-sogrid-edit&id='.$v->ID);echo $url;?>" class="imapper-edit-button"><?php echo __("Edit","sogrid_object_meta");?></a>
								<?php /*
								<a href="#javascript<?php //$url=admin_url('admin.php?page=woo-imagemapper&my_clone='.$v->ID);echo $url;?>" class="my_actions_<?php echo $this->id;?> imapper-edit-button" my_id="<?php echo $v->ID?>" my_name="my_object_id" my_action="clone"><?php echo __("Clone","sogrid_object_meta");?></a>
								*/ ?>
								<a href="#javascript" class="my_actions_<?php echo $this->id;?> imapper-delete-button" my_id="<?php echo $v->ID?>" my_name="my_object_id" my_action="delete"><?php echo __("Delete","sogrid_object_meta");?></a>
								<?php /*&nbsp;
